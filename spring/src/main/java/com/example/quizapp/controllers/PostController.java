...
public class PostController {
    ...
    // with request param way we can use this function with two ways: /posts and
    // /posts?userId={userId}. other parameters can be added also
    @GetMapping
    ...
    @PostMapping
    // direkt post objesinden değil, sadece userın IDsine ihtiyaç duyduğumuz için
    // request için gerekli olan bilgilerdeki classımızdan çağırıyopruz
    ...
    @PutMapping("/{postId}")
    // we want to update only specific places, not everything about the post. so we
    // are calling a request object for that
